...
var lowercase = function lowercase(string) {
    if (string[0].toLowerCase()) {
        var _lowercase = string[0].toLowerCase() + string.substring(1);
        return _lowercase;
    } else {
        return string;
    }
...
var ajaxRequest = function ajaxRequest(url, callback) {
...
var searchPokemon = function searchPokemon(url, callback) {
    var form = document.querySelector('#search-form');

    form.onsubmit = function (e) {
        e.preventDefault();
        var searchTerm = lowercase(serialize(document.forms[0]));
...
var appendHtml = function appendHtml(data) {
    console.log(data);

    // Pokemon sprite
    var spriteElement = document.querySelector('#pokemon-sprite');
    ...
    var typeElements = Array.from(document.querySelectorAll('.pokemon-type'));
    for (var _i = 0; _i < typeElements.length; ++_i) {
        if (types[_i]) {
            typeElements[_i].innerHTML = capitalise(types[_i]);
            typeElements[_i].style.backgroundColor = colours[types[_i]];
        } else {
            typeElements[_i].innerHTML = "";
            typeElements[_i].style.backgroundColor = 'transparent';
        }
    }

    // Pokemon abilities
    var abilitiesContainer = document.querySelector('#pokemon-abilities');
    abilitiesContainer.style.display = "block";

    // Remove any existing elements
    var children = Array.from(document.querySelectorAll('#pokemon-abilities p'));
    if (children) {
        for (var _i2 = 0; _i2 < children.length; ++_i2) {
            children[_i2].parentNode.removeChild(children[_i2]);
        }
    }

    for (var _i3 = 0; _i3 < data.abilities.length; ++_i3) {
        // Create paragraph element
        var element = document.createElement("p");
        abilitiesContainer.appendChild(element);

        // Format element text and append to innerHTML
        var ability = data.abilities['' + _i3].ability["name"];
        if (ability.includes("-")) {
            ability = ability.split("-");
            var abilityString = "";
            ability.forEach(function (e) {
...
(0, _data.searchPokemon)('https://pokeapi.co/api/v2/pokemon/', function (data) {
    var pokemonData = data;
    (0, _html.appendHtml)(pokemonData);
});

},{"./data":1,"./html":2,"normalize-css":5}],4:[function(require,module,exports){
var inserted = [];
...
},{"./normalize":6,"insert-css":4}],6:[function(require,module,exports){
